...
2. Сгенерировать матрицу смежности случайным образом
3. Заготовленная матрица смежности на 5 городов
4. Заготовленная матрица смежности на 20 городов''')
input_variant = o.input_value('int', 'Ваш выбор: ', lambda iv: 0 < iv < 5, 'Такого варианта нет, попробуйте ещё.')



if input_variant == 1:      # матрица смежности вручную
    ...
    for i in range(CITIES):
        for y in range(CITIES):
            if i == y:
                continue
            L[i][y] = o.input_value('float', 'Cities '+str(i)+'-'+str(y)+': ', lambda l: l >= 0,
                                    'Расстояние должно быть позитивным числом')
    np.fill_diagonal(L, 0)

if input_variant == 2:      # генерация случайной матрицы
    CITIES = o.input_value('int', 'Количество городов: ', lambda num: num > 2, 'Необходимо хотя бы 3 города')
...
if input_variant == 3:      # матрица на 5 городов
    CITIES = 5
...
print('Кратчайшее расстояние = ', int(BEST_DIST))
